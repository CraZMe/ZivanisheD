...
class DisplayBuffer:
    ...
    def Copy(self, FrameBuffer):
        if (int(FrameBuffer.FrameType.BitsPerPixel / 8) == 4):
            ...
            self.pixmap = QPixmap(qimage).scaled(500, 500)
...
class Listener(TIS.Imaging.IFrameQueueSinkListener):
    ...
    def FramesQueued(self, sink):
        ...
        if self.saveimage is True:
            ...
            TIS.Imaging.FrameExtensions.SaveAsJpeg(frame, "images/test.jpg", 75)
...
def OnResult(result):
    imgcontent = C.cast(result.FrameBuffer.GetIntPtr().ToInt64(),
                        C.POINTER(C.c_ubyte * result.FrameBuffer.FrameType.BufferSize))
